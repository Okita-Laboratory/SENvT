...
def permutation(x, max_segments=5, seg_mode="equal"):
    ...
    for i, pat in enumerate(x):
        if num_segs[i] > 1:
            ...
            ret[i] = pat
    return ret
def permutation_torch(x, max_segments=5, seg_mode='equal'):
    B, L, C = x.shape
    orig_steps = torch.arange(L)
    num_segs = torch.randint(1, L, size=(B, ))
    ret = torch.zeros_like(x)
    for i, pat in enumerate(x):
        if num_segs[i] > 1:
            splits = orig_steps.split(L//num_segs[i])
            warp = torch.cat([splits[p] for p in torch.randperm(len(splits))])
            ret[i] = pat[warp]
        else:
            ret[i] = pat
    return ret
...
#     random_warps = torch.normal(1., sigma, size=(B, knot+2, C))
#     warp_steps = (torch.ones(C, 1)*torch.linspace(0, L-1, knot+2)).T
...
#             time_warp = CubicSpline(warp_steps[:,dim], warp_steps[:,dim] * random_warps[i,:,dim])(orig_steps)
#             scale = (x.shape[1]-1)/time_warp[-1]
#             ret[i,:,dim] = interp(orig_steps, xp=torch.clip(torch.tensor(scale*time_warp, dtype=torch.float), min=0, max=L-1), fp=pat[:,dim]).T
#     return ret
